...
class Auth
{
    ...
    */
    ...
    public function activate($key)
    {
        ...
        $query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET isactive = ? WHERE uid = ?");
    ...
    */
    ...
    public function requestReset($email, $sendmail = NULL)
    {
        ...
        $query = $this->dbh->prepare("SELECT uid FROM {$this->config->table_users} WHERE email = ?");
    ...
    */
    ...
    protected function addUser($email, $password, $params = array(), &$sendmail)
    {
        ...
        if ($sendmail) {
            ...
            if ($addRequest['error'] == 1) {
                $query = $this->dbh->prepare("DELETE FROM {$this->config->table_users} WHERE uid = ?");
                $query->execute(array($uid));
                $return['message'] = $addRequest['message'];
        ...
        $query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET email = ?, password = ?, isactive = ? {$setParams} WHERE uid = ?");
    ...
    */
    ...
    {
        $query = $this->dbh->prepare("SELECT email, password, isactive FROM {$this->config->table_users} WHERE uid = ?");
        $query->execute(array($uid));
    ...
    */
    ...
    {
        $query = $this->dbh->prepare("SELECT * FROM {$this->config->table_users} WHERE uid = ?");
        $query->execute(array($uid));
    ...
    */
    ...
    public function resetPass($key, $password, $repeatpassword, $captcha = NULL)
    {
        ...
        $password = $this->getHash($password);
        $query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET password = ? WHERE uid = ?");
    ...
    */
    ...
    public function changeEmail($uid, $email, $password, $captcha = NULL)
    {
        ...
        $query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET email = ? WHERE uid = ?");
    ...
    */
    ...
    protected function deleteAttempts($ip, $all = false)
    {
        ...
        $query = $this->dbh->prepare("SELECT uid, expiredate FROM {$this->config->table_attempts} WHERE ip = ?");
        $query->execute(array($ip));
    ...
    */
    ...
    {
        $query = $this->dbh->prepare("SELECT password FROM {$this->config->table_users} WHERE uid = ?");
