...
class urlContentReader{
	...
	function parseCbsContent(){
		...
		while(preg_match('/<table class="lineScore ncaabBoxScore postEvent">/i',$parseContent))
		{
			...
			if(strpos($currentGame,'class="finalStatus" colspan="3">Final'))
			{
    	...
    	print_r($teams);
		$query="SELECT game.winid,game.points,game.loseid,users.user_name, region.region_rank AS winregion, team.uni_name AS winteam, team.seed AS winseed, game.winscore,
			...
			WHERE year.year=2016 AND ownership.lid='$leagueID'";
		$result = $mysqli->query($query) or die($mysqli->error.__LINE__);

		if($result->num_rows > 0) {
    		while($row = $result->fetch_assoc()) {
    			$arrDbGameResults[$row['winseed']][$row['loseseed']]['points'] = $row['points'];
			}
		}
		$query = '';
		foreach($this->arrGameResults as $game)
		{
			...
			{
				$query = $query . sprintf("INSERT INTO game SET winid = %s",$teams[$game['winteam']]);
			}
		}
		echo $query;
